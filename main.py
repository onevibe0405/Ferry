...
class ModBot(commands.Bot):
    ...
    async def setup_hook(self):
        ...
        try:
            connector = aiohttp.TCPConnector(
                limit=20,  # Reduced from 100 to prevent overwhelming hosting platforms
                limit_per_host=5,  # Reduced from 10
                ttl_dns_cache=600,  # Increased cache time
                use_dns_cache=True,
                keepalive_timeout=30,  # Reduced for hosting platforms
    ...
    @tasks.loop(minutes=5)  # Reduced frequency to 5 minutes to prevent rate limiting
    ...
    async def on_message(self, message):
        ...
        # Rate limiting check - skip processing if too many recent messages
        if hasattr(self, 'message_timestamps'):
            current_time = datetime.now().timestamp()
            self.message_timestamps = [t for t in self.message_timestamps if current_time - t < 10]
            if len(self.message_timestamps) > 50:  # Max 50 messages per 10 seconds
                return
