...
def update_bot_stats(bot):
    ...
    try:
        ...
        bot_stats['guilds'] = len(bot.guilds) if bot.is_ready() else 0
        
        # Calculate total unique users across all guilds
        ...
        print(f"📊 Stats updated: {bot_stats['guilds']} guilds, {bot_stats['users']} users, Status: {bot_stats['status']}")
    except Exception as e:
...
def save_data(data):
    """Save persistent data to JSON file with backup system"""
    try:
